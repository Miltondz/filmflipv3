...
export const uploadImageToFirebase = async (imageDataUrl: string, movieId: string): Promise<string> => {
  if (!auth.currentUser) throw new Error('User not authenticated');

  const storageRef = ref(storage, `shares/${auth.currentUser.uid}/${movieId}_${Date.now()}.png`);
